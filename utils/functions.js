...
const executeQueryFromFile = async (filepath) => {
    try {
        ...
        return fs.readFile(relativePath, 'utf-8', async (err, data) => {
            if (err) {
...
const executeProcedureWithParams = (procedureName, params) => {
    return new Promise((resolve, reject) => {
        connection.query(`CALL ${procedureName}(${params})`, (error, result) => {
            if (error) {
                console.error('Error executing procedure:', error);
                reject(error);
            } else {
